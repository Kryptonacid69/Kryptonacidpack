KryptonacidPack.base_values = {}

KryptonacidPack.misprintize_value_blacklist = {
	perish_tally = false,
	id = false,
	suit_nominal = false,
	base_nominal = false,
...
function KryptonacidPack.log_random(seed, min, max)
...
function KryptonacidPack.manipulate(card, args)
	...
	if not Card.no(card, "immutable", true) or (args and args.bypass_checks) then
		if not args then
			return KryptonacidPack.manipulate(card, {
			...
			local func = function(card)
				...
				if card.infinifusion then
					if card.config.center == card.infinifusion_center or card.config.center.key == "j_infus_fused" then
						calculate_infinifusion(card, nil, function(i)
							KryptonacidPack.manipulate(card, args)
						end)
					end
				end
				KryptonacidPack.manipulate_table(card, card, "ability", args)
				if card.base then
					KryptonacidPack.manipulate_table(card, card, "base", args)
				end
				if G.GAME.modifiers.cry_misprint_min then
					--card.cost = cry_format(card.cost / KryptonacidPack.log_random(pseudoseed('cry_misprint'..G.GAME.round_resets.ante),override and override.min or G.GAME.modifiers.cry_misprint_min,override and override.max or G.GAME.modifiers.cry_misprint_max),"%.2f")
					card.misprint_cost_fac = 1
						/ KryptonacidPack.log_random(
				...
				if caps then
					for i, v in pairs(caps) do
						if KryptonacidPack.is_big(v) then
							for i2, v2 in pairs(v) do
								if to_big(card.ability[i][i2]) > to_big(v2) then
									card.ability[i][i2] = KryptonacidPack.sanity_check(v2, KryptonacidPack.is_card_big(card))
								end
							end
						elseif KryptonacidPack.is_number(v) then
							if to_big(card.ability[i]) > to_big(v) then
								card.ability[i] = KryptonacidPack.sanity_check(v, KryptonacidPack.is_card_big(card))
							end
						end
					end
				end
			...
			if not KryptonacidPack.base_values[card.config.center.key] then
				KryptonacidPack.base_values[card.config.center.key] = {}
				for i, v in pairs(config) do
					if KryptonacidPack.is_number(v) and to_big(v) ~= to_big(0) then
						KryptonacidPack.base_values[card.config.center.key][i .. "ability"] = v
					elseif type(v) == "table" then
						for i2, v2 in pairs(v) do
							KryptonacidPack.base_values[card.config.center.key][i2 .. i] = v2
						end
					end
				end
			end
			if not args.bypass_checks and not args.no_deck_effects then
				KryptonacidPack.with_deck_effects(card, func)
			else
				func(card)
			end
			if card.ability.consumeable then
				for k, v in pairs(card.ability.consumeable) do
					card.ability.consumeable[k] = KryptonacidPack.deep_copy(card.ability[k])
				end
			end
			--ew ew ew ew
...
function KryptonacidPack.manipulate_table(card, ref_table, ref_value, args, tblkey)
	if ref_value == "consumeable" then
		return
	end
	for i, v in pairs(ref_table[ref_value]) do
		if
			KryptonacidPack.is_number(v)
			and KryptonacidPack.misprintize_value_blacklist[i] ~= false
		then
			local num = v
			if args.dont_stack then
				if
					KryptonacidPack.base_values[card.config.center.key]
					and (
						KryptonacidPack.base_values[card.config.center.key][i .. ref_value]
						or (ref_value == "ability" and KryptonacidPack.base_values[card.config.center.key][i .. "consumeable"])
					)
				then
					num = KryptonacidPack.base_values[card.config.center.key][i .. ref_value]
						or KryptonacidPack.base_values[card.config.center.key][i .. "consumeable"]
				end
			end
			if args.big ~= nil then
				ref_table[ref_value][i] = KryptonacidPack.manipulate_value(num, args, args.big, i)
			else
				ref_table[ref_value][i] = KryptonacidPack.manipulate_value(num, args, KryptonacidPack.is_card_big(card), i)
			end
		elseif i ~= "immutable" and type(v) == "table" and KryptonacidPack.misprintize_value_blacklist[i] ~= false then
			KryptonacidPack.manipulate_table(card, ref_table[ref_value], i, args)
		end
	end
end

function KryptonacidPack.manipulate_value(num, args, is_big, name)
	if not KryptonacidPack.is_number(num) then return end
	if args.func then
		...
		if args.min and args.max then
			...
			local new_value = KryptonacidPack.log_random(
	...
	if KryptonacidPack.misprintize_value_cap[name] then
		num = math.min(num, KryptonacidPack.misprintize_value_cap[name])
	end
	if KryptonacidPack.misprintize_bignum_blacklist[name] == false then
		num = to_number(num)
		return to_number(KryptonacidPack.sanity_check(num, false))
	end
	local val = KryptonacidPack.sanity_check(num, is_big)
