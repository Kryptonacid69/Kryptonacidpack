function KryptonacidPack.can_mods_load(...)
    local mods = {...}
...
function KryptonacidPack.optional_feature(key)
    for i, v in pairs(SMODS.Mods) do
        if v.can_load and v.spectrallib_features and KryptonacidPack.in_table(v.spectrallib_features, key) then return true end
    end
end
...
function KryptonacidPack.mod_score(amt) --good version
...
function KryptonacidPack.mod_blindsize(amt) --good version
