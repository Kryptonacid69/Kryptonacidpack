...
function KryptonacidPack.table_merge(t1, t2)
	local tbl = {}
...
function KryptonacidPack.get_highlighted_cards(areas, ignore, min, max, blacklist, seed)
	ignore.checked = true
...
function Card:is_food()
	...
	if food[self.config.center.key] or KryptonacidPack.safe_get(self.config.center, "pools", "Food") then
		return true
	end
end

function KryptonacidPack.cry_rankname_to_id(rankname)
...
---@param ... any
---@return table|false
function KryptonacidPack.safe_get(t, ...)
	local current = t
...
function KryptonacidPack.is_card_big(joker)
    ...
    -- im making bignums not work with KryptonacidPack. since i dont see the point
    -- could be changed but i dont feel like making 2 blacklists or making this mod use the KryptonacidPack table either
	if center.mod and not (KryptonacidPack or {})--[[.mod_whitelist[center.mod.name]--]] then
		return false
	end

	local in_blacklist = ((KryptonacidPack or {}).big_num_blacklist or {})[center.key or "Nope!"] or false

	return not in_blacklist --[[or
	       (center.mod and center.mod.id == "KryptonacidPack" and not center.no_break_infinity) or center.break_infinity--]]
end
...
function KryptonacidPack.no(center, m, key, no_no)
	if no_no then
		return center[m] or (G.GAME and G.GAME[m] and G.GAME[m][key]) or false
	end
	return KryptonacidPack.no(center, "no_" .. m, key, true)
end

function KryptonacidPack.deck_effects(card, func)
...
function KryptonacidPack.deep_copy(obj, seen)
	...
	for k, v in pairs(obj) do
		res[KryptonacidPack.deep_copy(k, s)] = KryptonacidPack.deep_copy(v, s)
	end
	return res
end

-- generate a random edition (e.g. Antimatter Deck)
function KryptonacidPack.poll_random_edition()
...
function KryptonacidPack.random_consumable(seed, excluded_flags, banned_card, pool, no_undiscovered)
	...
	while true do
		...
		if selection.discovered or not no_undiscovered then
			for k, v in pairs(excluded_flags) do
				if not KryptonacidPack.no(selection, v, key, true) then
...
function KryptonacidPack.pluralize(str, vars)
...
function KryptonacidPack.advanced_find_joker(name, rarity, edition, ability, non_debuff, area)
	local jokers = {}
	...
	if not area or area == "j" then
		for k, v in pairs(G.jokers.cards) do
			if v and type(v) == "table" and (non_debuff or not v.debuff) then
				...
				if
					edition
					and (v.edition and v.edition.key == edition) --[[ make this use KryptonacidPack.safe_get later? if it's possible anyways]]
				then
					check = check + 1
				end
				if rarity then
...
function KryptonacidPack.with_deck_effects(card, func)
	if not card.added_to_deck then
		return func(card)
	else
		card.from_quantum = true
...
if not KryptonacidPack.can_mods_load({"Cryptid", "Cryptlib"}) then
...
function KryptonacidPack.pulse_flame(duration, intensity) -- duration is in seconds, intensity is in idfk honestly, but it increases pretty quickly
...
KryptonacidPack.scoring_window_pulse_targets = {
	{G.C.UI_MULT, G.C.RED},
	{G.C.UI_CHIPS, G.C.BLUE},
}

function KryptonacidPack.pulse_scoring_window_colors(new_color, fade_in, hold, fade_out)
...
function KryptonacidPack.get_next_tag()

end

function KryptonacidPack.is_number(x)
	return type(x) == "number" or (type(x) == "table" and is_number(x)) or (is_big and is_big(x))
end
function KryptonacidPack.is_big(x)
	return (type(x) == "table" and is_number(x)) or (is_big and is_big(x))
end

function KryptonacidPack.clamp(x, min, max)
