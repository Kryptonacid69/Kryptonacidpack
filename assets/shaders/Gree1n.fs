...
vec4 Green_color = vec4(0., 0., 255., 255.) / 255.; //changes color - me
...
bool line(vec2 uv, float offset, float width) {
    ...
    offset = offset * 1.4 + 0.2 * Green.x; // these change the position of lines
    width = 0.2 * Green.x; //  when hovered over i think and their width

    float min_y = offset;
    float max_y = offset + width;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    pixel = vec4(Green_color.rgb * avg + tex.rgb * tex.a, pixel.a);
